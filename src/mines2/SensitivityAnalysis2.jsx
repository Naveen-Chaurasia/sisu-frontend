...
function Spinner() {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:300, flexDirection:"column", gap:12 }}>
      ...
      <span style={{ fontSize:13, color:THEME.muted }}>Running sensitivity analysis…</span>
    </div>
  );
}
...
function CustomTooltip({ active, payload, label, metricFmt, variation }) {
      ...
      {payload.map(p => (
        <div key={p.dataKey} style={{ display:"flex", justifyContent:"space-between", gap:20, marginBottom:2 }}>
          <span style={{ color:THEME.muted }}>
            {p.dataKey === "low_change" ? `−${(variation*100).toFixed(0)}% input` : `+${(variation*100).toFixed(0)}% input`}
          </span>
...
export default function SensitivityAnalysis2({ mineId, mineColor }) {
          ...
          <div style={{
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
              ...
              <div style={{ display:"flex", gap:16 }}>
                {[
                  { color:"#1e7093", label:`Input +${(variation*100).toFixed(0)}%` },
                  { color:"#f97316", label:`Input −${(variation*100).toFixed(0)}%` },
                ].map(l => (
            ...
            <ResponsiveContainer width="100%" height={chartData.length * 54 + 40}>
              ...
              >
                ...
                />
                <Tooltip content={<CustomTooltip metricFmt={metricDef.fmt} variation={variation} />} />
                <ReferenceLine x={0} stroke={THEME.primaryDark} strokeWidth={2} />
                <Bar dataKey="low_change"  maxBarSize={20} radius={[0,4,4,0]}>
                  {chartData.map((entry, i) => (
                    <Cell key={i} fill={entry.low_change >= 0 ? "#1e7093" : "#f97316"} />
                  ))}
                </Bar>
                <Bar dataKey="high_change" maxBarSize={20} radius={[0,4,4,0]}>
                  {chartData.map((entry, i) => (
                    <Cell key={i} fill={entry.high_change >= 0 ? "#1e7093" : "#f97316"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
