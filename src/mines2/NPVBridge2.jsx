...
// ── Format: DCF values are $mm; show as $B or $mm ────────────────────────────
function fmtB(v) {
  if (v == null || isNaN(v)) return "—";
  const abs  = Math.abs(v);
  const sign = v < 0 ? "−" : "";
  if (abs >= 1000) return `${sign}$${(abs / 1000).toFixed(2)}B`;
  if (abs >= 1)    return `${sign}$${abs.toFixed(1)}mm`;
...
// ── Build waterfall data from lifetime aggregates ─────────────────────────────
function buildWaterfall(years) {
  const sum = key => years.reduce((acc, y) => acc + (y[key] ?? 0), 0);

  const gr     = sum("gross_revenue");
  const roy    = sum("royalty");                // stored negative
  const nr     = sum("net_revenue");
  const opex   = sum("operating_costs");        // stored negative
  const ebitda = sum("ebitda");
  const tax    = sum("income_tax");             // stored negative
  const capex  = sum("capex");                  // stored negative
  const fcf    = sum("free_cash_flow");
  const npv    = sum("discounted_cf");
  const disc   = npv - fcf;                    // negative discount effect

  // spacer = transparent offset; value = bar height; type drives color
  // For DEDUCTIONS: bar floats above running-total-after
  // For TOTALS: bar starts from 0
  const steps = [
...
function WaterfallChart({ steps, accentColor }) {
  ...
  return (
    <div style={{ background:THEME.card, borderRadius:8, padding:"24px 20px 16px",
      ...
      boxShadow:"0 1px 6px rgba(0,0,0,0.05)", marginBottom:24 }}>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={steps} margin={{ left:10, right:20, top:20, bottom:40 }}
          barCategoryGap="20%">
          ...
          {/* Visible value bar — colored by type */}
          <Bar dataKey="value" stackId="bridge" radius={[4,4,0,0]} maxBarSize={48}>
            ...
            <LabelList dataKey="running" position="top"
              formatter={v => fmtBShort(v)}
...
const CARD_DEFS = [
  ...
  },
  {
    ...
    sub:   "Roy + OPEX + Tax + CAPEX",
    accent:"#dc2626",
    bg:    "linear-gradient(135deg,#fff5f5 0%,#fef2f2 100%)",
    textClr:"#9f1239",
    icon: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {cards.map(({ label, value, sub, color }) => (
        <div key={label} style={{
          ...
          <div style={{ fontSize:11, fontWeight:700, color:THEME.muted,
            letterSpacing:0.5, textTransform:"uppercase", marginBottom:6 }}>{label}</div>
...
function BreakdownTable({ steps, gr }) {
  const LABELS = {
    income:   { color:"#0ea5e9" },
    cost:     { color:"#f87171" },
    subtotal: { color:THEME.primaryDark },
    final:    { color:"#7c3aed" },
  };
  return (
    <div style={{ background:THEME.card, borderRadius:8, overflow:"hidden",
      ...
      <div style={{ overflowX:"auto" }}>
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
          ...
          <tbody>
            {steps.map((s, i) => {
              const meta = LABELS[s.type] || LABELS.subtotal;
              ...
              return (
                <tr key={i} style={{ borderBottom:"1px solid #f8fafc" }}
                  ...
                  <td style={{ padding:"10px 16px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <div style={{ width:8, height:8, borderRadius:2,
                        background:TYPE_COLOR[s.type]||THEME.primary }}/>
                  ...
                  <td style={{ padding:"10px 16px", textAlign:"right", color:THEME.primaryDark, fontWeight:600 }}>
                    {fmtB(s.running)}
                  </td>
                  <td style={{ padding:"10px 16px", textAlign:"right", color:THEME.muted }}>
                    {fmtPct(s.running, gr)}
                  </td>
                </tr>
              );
...
function Tab({ label, active, onClick, sub }) {
  return (
    <button onClick={onClick} style={{
      ...
      {label}{sub&&<span style={{ fontSize:9, opacity:.7, marginLeft:5 }}>{sub}</span>}
    </button>
  );
}

// ── Main ──────────────────────────────────────────────────────────────────────
export default function NPVBridge2({ mineId, mineColor }) {
  ...
  const bridge = dcfData?.years?.length ? buildWaterfall(dcfData.years) : null;
    ...
    <div style={{ padding:"28px 32px" }}>
      ...
      {activeScenObj && (
        <div style={{ marginBottom:16, fontSize:11, color:THEME.muted, display:"flex", alignItems:"center", gap:6 }}>
          ...
          <span>—</span>
          <span>{activeScenObj.sheet_name || (SCEN_LABELS[activeScenObj.scenario]||activeScenObj.scenario)}</span>
