  ...
  const MODES = [
    {
      ...
      desc: "Configure your policy using goal presets, parameter dropdowns, magnitude slider, and target year. Full control over every model variable.",
      best: "Precise parameter control",
    ...
    },
    {
      ...
      desc: "Describe your policy in plain English. Sustain360 will interpret your intent, map it to the right model parameters, and run the simulation automatically.",
      best: "Quick exploration & ideas",
    ...
    },
    {
      ...
      desc: "View Scope 1 (direct combustion), Scope 2 (purchased electricity), and Scope 3 (value chain) transport emissions broken down by mode — aligned with GHG Protocol.",
      best: "GHG Protocol reporting",
      cta: "Explore Scopes",
    },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "14px 32px", textAlign: "center" }}>
        <h1 style={{ fontSize: 17, fontWeight: 700, color: "#0f172a", letterSpacing: -0.3, margin: 0 }}>National Emission Baselining and Decarbonization Modeling</h1>
      </div>

      <div style={{ maxWidth: 820, margin: "0 auto", padding: "56px 32px" }}>
        {/* Welcome heading */}
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <p style={{ fontSize: 15, color: "#64748b", lineHeight: 1.75, maxWidth: 540, margin: "0 auto" }}>
            Design, simulate, and evaluate national transport decarbonization strategies with Sustain360 AI-powered climate impact insights.
          </p>
        </div>
        ...
        <div style={{ display: "flex", gap: 20 }}>
          {MODES.map(m => (
            <button key={m.id} onClick={() => onSelect(m.id)} style={{
              flex: 1, background: "#fff", border: "2px solid #e2e8f0", borderRadius: 18,
              padding: "32px 28px", cursor: "pointer", fontFamily: "inherit",
              display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 16,
              textAlign: "left", transition: "all 0.18s", boxShadow: "0 2px 8px rgba(0,0,0,0.05)",
            }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "#1e7093"; e.currentTarget.style.boxShadow = "0 8px 32px rgba(30,112,147,0.18)"; e.currentTarget.style.transform = "translateY(-3px)"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "#e2e8f0"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.05)"; e.currentTarget.style.transform = "none"; }}
            >
              <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg, rgba(26,101,133,0.1), rgba(30,112,147,0.12))", display: "flex", alignItems: "center", justifyContent: "center", color: "#1a6585" }}>
                {m.icon}
              </div>
              <div>
                <div style={{ fontSize: 18, fontWeight: 700, color: "#0f172a", marginBottom: 10 }}>{m.title}</div>
                <div style={{ fontSize: 13.5, color: "#64748b", lineHeight: 1.7 }}>{m.desc}</div>
              </div>
              <div style={{ fontSize: 11.5, fontWeight: 700, color: "#1e7093", background: "rgba(30,112,147,0.08)", borderRadius: 20, padding: "4px 14px" }}>
                Best for: {m.best}
              </div>
              <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 700, color: "#1a6585" }}>
                {m.cta}
            ...
            {mode === "natural" ? (
              ...
              <div style={{
                ...
                <div style={{ paddingTop: 14, borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end" }}>
                  <button onClick={runNL} disabled={loading || !nlText.trim()} style={{
                    ...
                    {loading
                      ? <><span style={{ display: "inline-block", width: 15, height: 15, border: "2px solid #94a3b860", borderTop: "2px solid #94a3b8", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> Analyzing…</>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: 14, borderTop: "1px solid #f1f5f9" }}>
                ...
                <button onClick={() => runSimulation()} disabled={loading} style={{
                  ...
                  {loading
                    ? <><span style={{ display: "inline-block", width: 15, height: 15, border: "2px solid #94a3b860", borderTop: "2px solid #94a3b8", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> Running…</>
                    : <><span style={{ fontSize: 15 }}>▶</span> Run Simulation</>}
                </button>
              </div>
            </div>
          ...
          <div style={{
            ...
            {!result && (
              <div style={{
                ...
                {loading ? (
                  <>
                    <div style={{ display: "inline-block", width: 36, height: 36, border: "3px solid rgba(30,112,147,0.2)", borderTop: "3px solid #1e7093", borderRadius: "50%", animation: "spin 0.9s linear infinite", marginBottom: 16 }} />
